...
void AudioHardware::AudioSessionOutLPA::bufferAlloc( )
{
    ...
    for (int i = 0; i < mInputBufferCount; i++) {
        pmem_buf = memBufferAlloc(mInputBufferSize , &pmem_fd);
        memset(&pmem_info, 0, sizeof(msm_audio_pmem_info));
        ALOGV("Registering PMEM with fd %d and address as %x", pmem_fd, pmem_buf);
        pmem_info.fd = pmem_fd;
        pmem_info.vaddr = pmem_buf;
        if ( ioctl(afd, AUDIO_REGISTER_PMEM, &pmem_info) < 0 ) {
            ALOGE("Registration of PMEM with the Driver failed with fd %d and memory %x",
...
void* AudioHardware::AudioSessionOutLPA::memBufferAlloc(int nSize, int32_t *pmem_fd)
{
    int32_t pmemfd = -1;
    void  *pmem_buf = NULL;
    void  *local_buf = NULL;
    ...
    if (pmemfd < 0) {
        ALOGE("memBufferAlloc failed to open pmem_audio");
        *pmem_fd = -1;
        return pmem_buf;
    }
    ...
    if ( NULL == pmem_buf) {
        ALOGE("memBufferAlloc failed to mmap");
        *pmem_fd = -1;
        return NULL;
    }
    ...
    if (NULL == local_buf) {
        ...
        munmap(pmem_buf, mInputBufferSize);
        close(pmemfd);
        return NULL;
    }
    ...
    *pmem_fd = pmemfd;
    ...
    ALOGV("The PMEM that is allocated is %d and buffer is %x", pmemfd, (unsigned int)pmem_buf);
...
void AudioHardware::AudioSessionOutLPA::bufferDeAlloc()
{
    ...
    while (!mEmptyQueue.empty())  {
        ...
        if (ioctl(afd, AUDIO_DEREGISTER_PMEM, &pmem_info) < 0) {
            ALOGE("PMEM deregister failed");
        }
        ALOGV("Unmapping the address %u, size %d, fd %d from empty Q",pmemBuffer.memBuf,pmemBuffer.bytesToWrite,pmemBuffer.memFd);
        munmap(pmemBuffer.memBuf, mInputBufferSize);
        ALOGV("closing the pmem fd");
        close(pmemBuffer.memFd);
        // free the local buffer corresponding to pmem buffer
        free(pmemBuffer.localBuf);
        ALOGV("Removing from empty Q");
        mEmptyQueue.erase(it);
    }
    ...
    while(!mFilledQueue.empty()){
        ...
        ALOGV("Unmapping the address %u, size %d, fd %d from filled Q",pmemBuffer.memBuf,mInputBufferSize,pmemBuffer.memFd);
        munmap(pmemBuffer.memBuf, mInputBufferSize);
        ALOGV("closing the pmem fd");
        close(pmemBuffer.memFd);
        // free the local buffer corresponding to pmem buffer
