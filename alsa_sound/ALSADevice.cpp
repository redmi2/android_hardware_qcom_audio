...
ALSADevice::ALSADevice() {
    ...
    for (i = 0; i < MAX_SOUND_CARDS; i++) {
		...
		strstr((const char*)mSndCardInfo.name, "msmsamarium")) {
                break;
            }
        }
    ...
    if (isExternalModem() && (inCallDevSwitch == true)) {
       ...
       /* Get rx acdb id */
        ...
        if (csd_enable_device_config == NULL) {
            ...
            err = csd_enable_device_config(mRxACDBID, mTxACDBID);
            if (err < 0) {
...
void ALSADevice::setVoipConfig(int mode, int rate)
{
    ...
    snprintf(setValues[1], (4*sizeof(char)), "%d", rate);
...
void ALSADevice::setVoipEvrcMinMaxRate(int minRate, int maxRate)
{
    ...
    snprintf(setValues[0], (4*sizeof(char)), "%d", minRate);
    snprintf(setValues[1], (4*sizeof(char)), "%d", maxRate);
