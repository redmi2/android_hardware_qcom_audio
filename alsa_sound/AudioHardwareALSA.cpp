...
{
    Mutex::Autolock autoLock(mLock);
    ALOGE("openOutputStream: devices 0x%x channels %d sampleRate %d flags",
    ...
    if ((flags & AUDIO_OUTPUT_FLAG_DIRECT) &&
        ...
        if (0 == *channels) {
            ...
            ALOGE(" if 0 == *channels channels %d alsa_handle.channels %d", *channels, alsa_handle.channels);
            if (alsa_handle.channels > 6) {
                alsa_handle.channels = 6;
            }
            *channels = audio_channel_out_mask_from_count(alsa_handle.channels);
            ALOGE(" if 0 == *channels channels %d", *channels);
        } else {
            alsa_handle.channels = AudioSystem::popCount(*channels);
            ALOGE(" else channels %d alsa_handle.channels %d", *channels, alsa_handle.channels);
        }
        ...
        alsa_handle.rxHandle = 0;
        alsa_handle.ucMgr = mUcMgr;
        ALOGE("alsa_handle.channels %d alsa_handle.sampleRate %d",alsa_handle.channels,alsa_handle.sampleRate);
